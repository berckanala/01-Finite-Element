...
class Element:
    ...
    def get_B_matrix(self, nodes, xi, eta):
        ...
        for i in range(self.num_nodes):
            J[0, 0] += dN_dxi_func[i] * coords[i, 0]
            J[0, 1] += dN_dxi_func[i] * coords[i, 1]
            J[1, 0] += dN_deta_func[i] * coords[i, 0]
            J[1, 1] += dN_deta_func[i] * coords[i, 1]
        ...
        dN_dx = Jinv[0, 0] * dN_dxi_func + Jinv[0, 1] * dN_deta_func
        dN_dy = Jinv[1, 0] * dN_dxi_func + Jinv[1, 1] * dN_deta_func
    ...
    def get_load_vector(self, nodes, alpha):
        ...
        f_local = np.zeros(self.num_nodes)

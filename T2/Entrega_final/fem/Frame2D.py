...
class Frame2D:
    def __init__(self, coord_inicial, coord_final, E, A, I, printSummary=True):
    ...
    def __str__(self):
        return f"Frame2D: {self.coord_inicial.name} -> {self.coord_final.name}"
    ...
    def stiffness_matrix_basic(self):
        kb=np.array([[self.A*self.E/self.longitud, 0, 0],
                      [0, 4*self.E*self.I/self.longitud, 2*self.E*self.I/self.longitud],
                      [0, 2*self.E*self.I/self.longitud, 4*self.E*self.I/self.longitud]])
        return kb
        
    def basicLocalTransformation(self):
        Tbl=np.array([[-1,0,0,1,0,0],
                      [0,1/self.longitud,1,0,-1/self.longitud,0],
                      [0,1/self.longitud,0,0,-1/self.longitud,1]])
        return Tbl
